...
public class TriggerListenerLogMonitor implements TriggerListener {
    ...
    public void triggerFired(Trigger trigger, JobExecutionContext context) {
        // 检查调度任务 是否已被执行 若已执行 则抛弃不执行
        System.out.println("TriggerListenerLogMonitor类:" + context.getTrigger().getKey().getName() + "  " + context.getTrigger().getKey().getGroup() + " " + context.getJobDetail().getKey() + " " + context.getJobDetail().getJobClass() + " 被执行");
    }
    ...
    public void triggerComplete(Trigger trigger, JobExecutionContext context, Trigger.CompletedExecutionInstruction triggerInstructionCode) {
        System.out.println("Job执行完毕,Trigger完成" + context.getJobDetail().getJobClass());
    }
}
